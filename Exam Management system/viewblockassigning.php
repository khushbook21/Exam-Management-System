...
<div id="main" style="margin-left:0%">
  ...
  <tr><th>Block</th><th>Seat</th><th>Class</th></tr>

  <?php 
    $conn = mysqli_connect("localhost", "root", "", "ut_db");
    $sql="select * from blockassign";
    $select=mysqli_query($conn, $sql);
    $count=mysqli_num_rows($select);
    while($row=mysqli_fetch_array($select))
    {
    echo "<tr><td>".$row['block']."</td><td>".$row['seat']."</td><td>".$row['class']."</td></tr>";
    }
  ?>
  </table>

    <center><a href ='blockassigning.php'><h4>Add New Blocks</h4></a></center>
	</div>	
	</section>
